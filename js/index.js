...
$("#botonPartidas").click(function() {
    ...
    if (sessionStorage.getItem('nick') == undefined) {
        ...
        limpiarValids();
        $("#cabeceraPartida").text("Crear Partida");
        ...
        $("#fondo").removeClass("baseNormal");
        $("#fondo").addClass("baseDesenfocada");
        $("#btnEditarPartida").css('display', 'none');
        $("#btnCrearPartida").css('display', 'inline');
    }
});
...
$("#btnEditarPartida").click(valEditPart);
...
function limpiarValids() {
    ...
    $("#nombrePersEdit").removeClass("falloValid");
    $("#lstTipoPersEdit").removeClass("falloValid");
    $("#lstArmasPersEdit").removeClass("falloValid");
    $("#nivPersEdit").removeClass("falloValid");
    $("#descrPersEdit").removeClass("falloValid");
}
...
function rellenaComboArmas(oArrayArmas) {
    ...
    $("#lstArmasPersEdit").empty();
    $('<option value="0">Escoja un arma</option>').appendTo("#lstArmasPersEdit");

    for (var i = 0; i < oArrayArmas.length; i++) {
        $('<option value="' + oArrayArmas[i].arma_id + '">' + oArrayArmas[i].arma_nombre + ' de tipo ' + oArrayArmas[i].arma_tipo + '</option>').appendTo("#lstArmasPersEdit");
    }
}
...
function rellenaComboTipoPersonaje(oArrayTipoPersonajeBusc) {
    ...
    $('<option value="0">Escoja un tipo de personaje</option>').appendTo("#lstTipoPersEdit");
    ...
    for (var i = 0; i < oArrayTipoPersonajeBusc.length; i++) {
        $('<option value="' + oArrayTipoPersonajeBusc[i].tipo_personaje + '">' + oArrayTipoPersonajeBusc[i].tipo_personaje + '</option>').appendTo("#lstTipoPersEdit");
    }
}
...
function respuestaBuscaPersonajes(oXML) {
    ...
    let posPers;
    let persEncotrado = oXML.getElementsByTagName("personaje");
    let table = "";

    if(persEncotrado.length == 0){
        table = "<h4>No hay partidas con esos datos</h4>";
    }

    else
        {
            table = "<table border='2' class='tabla'>";
...
function respuestaBuscaEsc(oXML) {
    ...
    let posEsc;
    let escEncotrado = oXML.getElementsByTagName("escenario");
    let table= "";

    if(escEncotrado.length == 0){
        table = "<h4>No hay escenarios con esos datos</h4>";
    }
    else
        {
            table = "<table border='2' class='tabla'>";
    ...
    if (oDatos.error) {
        ...
        if (oDatos.personaje.length == 0) {
            ...
            for (var i = 0; i < oDatos.personaje.length; i++) {
                ...
                sTabla += "<td style='text-align:center'><i id='bEditP' class='fa fa-edit bnEdit' data-id-personaje='" + oDatos.personaje[i].personaje_id + "' ></i></td>";
                sTabla += "<td style='text-align:center'><i id='bEraseP' class='fa fa-eraser bnErase' data-id-personaje='" + oDatos.personaje[i].personaje_id + "' ></i></td>";
        ...
        // Agregar manejador de evento
        let oBotonesEdicion = document.querySelectorAll("#tablaPersonajesLogueado .fa-edit");
        let oBotonesEliminar = document.querySelectorAll("#tablaPersonajesLogueado .fa-eraser");

        for (let i = 0; i < oBotonesEdicion.length; i++) {
            oBotonesEdicion[i].addEventListener("click", cargarFormEditPers, false);
        }

        for (let i = 0; i < oBotonesEliminar.length; i++) {
            oBotonesEliminar[i].addEventListener("click", eliminarPersonaje, false);
        }

    }
}

function cargarFormEditPers() {
...
function cargarPersEdit() {

    let nIdPersEdit = event.target.dataset.idPersonaje;
    // alert(nIdPersEdit);

    let sDatos = "idEditar=" + JSON.stringify(nIdPersEdit);
    sessionStorage.setItem('persEditar', event.target.dataset.idPersonaje);

    $.post("./php/getPersEditar.php",
        sDatos,
        cargarEditarPers,
        "json");
...
function cargarEditarPers(datos, textStatus, jqXHR) {

    //Metemos los datos del personaje a editar en los inputs del formulario

    document.getElementById("nombrePersEdit").value = datos[0].personaje_nombre;
    document.getElementById("lstTipoPersEdit").value = datos[0].tipo_personaje;
    document.getElementById("lstArmasPersEdit").value = datos[0].personaje_arma;
    document.getElementById("nivPersEdit").value = datos[0].personaje_xp;
    document.getElementById("descrPersEdit").value = datos[0].personaje_observ;
}
...
function valEditPersonaje() {
    ...
    // Validación nombre personaje
    let sNombrePersonaje = frmEditPersonaje.nombrePersEdit.value.trim();
    let oRegCreaNombPers = /^(([A-Za-záéíóúÁÉÍÓÚ1-9-_])|(\s)){4,15}$/;

    if (!oRegCreaNombPers.test(sNombrePersonaje)) {
        bCorrecto = false;
        sError = "\n- El nombre del personaje es incorrecto";
        frmEditPersonaje.nombrePersEdit.classList.add("falloValid");
        frmEditPersonaje.nombrePersEdit.focus();
    } else {
        frmEditPersonaje.nombrePersEdit.classList.remove("falloValid");
    }

    // Se ha elegido tipo de personaje
    let sTipoPers = frmEditPersonaje.lstTipoPersEdit.value.trim();

    if (sTipoPers == 0) {
        bCorrecto = false;
        sError += "\n- No ha escogido tipo de personaje";
        frmEditPersonaje.lstTipoPersEdit.classList.add("falloValid");
    } else {
        frmEditPersonaje.lstTipoPersEdit.classList.remove("falloValid");
    }

    // Se ha escogido arma
    let sArmaPers = frmEditPersonaje.lstArmasPersEdit.value.trim();

    if (sArmaPers == 0) {
        bCorrecto = false;
        sError += "\n- No ha elegido arma para su personaje";
        frmEditPersonaje.lstArmasPersEdit.classList.add("falloValid");
    } else {
        frmEditPersonaje.lstArmasPersEdit.classList.remove("falloValid");
    }

    //Se ha rellenado el campo descripción
    let sDescPersonaje = frmEditPersonaje.descrPersEdit.value.trim();

    if (sDescPersonaje == '') {
        bCorrecto = false;
        sError += "\n- No ha descrito a su personaje";
        frmEditPersonaje.descrPersEdit.classList.add("falloValid");
    } else {
        frmEditPersonaje.descrPersEdit.classList.remove("falloValid");
    }
...
function editPersonaje() {
    let oAjax = instanciarXHR();
    let sEnviarPersEdit = "nombPersonaje=" + frmEditPersonaje.nombrePersEdit.value.trim();
    sEnviarPersEdit += "&tipoPersonaje=" + frmEditPersonaje.lstTipoPersEdit.value.trim();
    sEnviarPersEdit += "&armaPersonaje=" + frmEditPersonaje.lstArmasPersEdit.value.trim();
    sEnviarPersEdit += "&nivelPersonaje=" + frmEditPersonaje.nivPersEdit.value.trim();
    sEnviarPersEdit += "&descrPersonaje=" + frmEditPersonaje.descrPersEdit.value.trim();
    sEnviarPersEdit += "&idPersonaje=" + sessionStorage.getItem('persEditar');
    sEnviarPersEdit = encodeURI(sEnviarPersEdit);

    //Configurar la llamada --> Asincrono por defecto
    oAjax.open("POST", encodeURI("./php/ediciones/editPersonaje.php"));

    //Asociar manejador de evento de la respuesta
    oAjax.addEventListener("readystatechange", accionEditPersonaje, false);

    // Cabecera POST
    oAjax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

    //Llamamos
    oAjax.send(sEnviarPersEdit);
}

function accionEditPersonaje() {
    var oAjax = this;
    // Proceso la respuesta cuando llega
    if (oAjax.readyState == 4 && oAjax.status == 200) {
        var oRespuesta = JSON.parse(oAjax.responseText);
...
function eliminarPersonaje() {
    let nIdEliminar = event.target.dataset.idPersonaje;
    // alert(nIdEliminar);

    let sDatos = "idEliminar=" + JSON.stringify(nIdEliminar);
    // sessionStorage.setItem('persEliminar', event.target.dataset.idPersonaje);
    if (confirm("¿Está seguro de querer eliminar el personaje?")){
        $.post("./php/ediciones/elimPersonaje.php",
            sDatos,
            resultadoElimPers,
            "json");
    }
...
function resultadoElimPers(oDatos, sStatus, oXHR) {
...
function cargarTablaPartidas(oTabla) {
    let sNick = sessionStorage.getItem('nick');
    ...
    let oBotonesEdicionPart = document.querySelectorAll("#tablaPartidasLogueado .fa-edit");

    for (let i = 0; i < oBotonesEdicionPart.length; i++) {
        oBotonesEdicionPart[i].addEventListener("click", cargarFormEditPart, false);
    }
}

function cargarFormEditPart() {
    // alert("Prueba de comprobación");
    $("form").hide();
    $("#popup1").css("display", "none");
    $("#avisos").removeClass("superior");
...
function cargarEditPart() {
    let nIdPartEdit = event.target.dataset.idPartida;
    // alert(nIdPartEdit);

    let sDatos = "idPartEditar=" + JSON.stringify(nIdPartEdit);
    sessionStorage.setItem('partEditar', event.target.dataset.idPartida);

    $.get("./php/getPartEditar.php",
        sDatos,
        cargarPartEdit,
        "json");
}

function cargarPartEdit(datos, textStatus, jqXHR) { //v1
    document.getElementById("fechaPartida").value = datos[0].partida_fecha;
    document.getElementById("lstEscePartida").value = datos[0].escenario_id;

    document.getElementById("nivelPersonaje").value = datos[0].partida_nivel;
    document.getElementById("lstEstadoPartida").value = datos[0].partida_estado;
    document.getElementById("descripcionPartida").value = datos[0].partida_observ;

    let listaOptions = document.querySelectorAll("#lstJugPartida option");
    let oSelect = document.querySelectorAll("#lstJugPartida");
    for (let i = 0; i < datos.length; i++) {
        if (datos[i].es_master = '0') {
            for (let j = 0; j < listaOptions.length; j++) {
                if (listaOptions[j].value == datos[i].jugador_id) {
                    listaOptions[j].selected = "selected";
                }
    ...
    $("#btnEditarPartida").css('display', 'inline');
    $("#btnCrearPartida").css('display', 'none');
    $("#cabeceraPartida").text("Editar Partida");
}

function valEditPart() {
    let sError = "";
    let bCorrecto = true; //Creamos el formulario como válido
...
function editarPartida() {
    //Creamos vacío el arrayJugEdit
    let arrayJugEdit = [];

    //Por cada elemento seleccionado del combo de jugadores
    for (let i = 0; i < frmPartidas.lstJugPartida.selectedOptions.length; i++) {
        //lo metemos en el arrayJugEdit
        arrayJugEdit.push(frmPartidas.lstJugPartida.selectedOptions[i].value);
    }

    var oPartida = {
        idPartida: sessionStorage.getItem('partEditar'),
        fechaEdit: frmPartidas.fechaPartida.value.trim(),
        escEdit: frmPartidas.lstEscePartida.value.trim(),
        jugEdit: arrayJugEdit,
        nivelEdit: frmPartidas.nivelPersonaje.value.trim(),
        estadoEdit: frmPartidas.lstEstadoPartida.value.trim(),
        descrEdit: frmPartidas.descripcionPartida.value.trim()

    };

    var sDatos = "partidaEditar=" + JSON.stringify(oPartida);

    $.post("./php/ediciones/editPartida.php",
        sDatos,
        resEditPart,
        "json");
}

function resEditPart(oDatos, sStatus, oXHR) {
